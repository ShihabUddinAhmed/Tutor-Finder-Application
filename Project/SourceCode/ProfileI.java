...
import javax.swing.*;
import java.sql.*;
 public class ProfileI extends JFrame implements ActionListener
 {
	 ...
	 public ProfileI(String ID)
	 {
		 ...
		 try
		{
			...
			String sq="SELECT * FROM `account` WHERE `INSTITUTION_ID` = "+ID;
			...
			{
				 IDS=rSet.getString("INSTITUTION_ID");
				...
				catch(Exception ex)
				{
				 ...
				 s=rSet.getString("INSTITUTION_NAME");
		...
		catch(Exception ex)
		{
		 ...
		 add(l);add(b3);add(tf);add(b);add(l3);add(tf1);add(b1);add(l5);add(tf3);add(l2);add(b5);add(l6);add(tf4);add(b6);add(b7);
		 ...
		 {try
		 {
			 ...
			 if(sig.equals("Edit Name"))
			 {
				 ...
				 else
				 {
					 ...
					 i=du.ddlOperation("UPDATE `institution` SET `INSTITUTION_NAME` = '"+inputValue+"' WHERE `institution`.`INSTITUTION_ID` = "+IDS+";");
				 if(i==1)
				 {
			 ...
			 else if(sig.equals("Edit Phone-Number"))
			 {
				 ...
				 {
				 i=du.ddlOperation("UPDATE `institution` SET `PHONE_NUMBER` = '"+inputValue+"' WHERE `institution`.`INSTITUTION_ID` = "+IDS);
				 if(i==1)
				 {
					 JOptionPane.showMessageDialog(null,
								"Phone-Number Edited",
			 ...
			 else if(sig.equals("Edit Address"))
			 {
				 ...
				 try
				{
					...
					{
						 flag=true;
						 i=du.ddlOperation("UPDATE `institution` SET `ADDRESS_ID` = '"+rSet.getString("ADDRESS_ID")+"' WHERE `institution`.`INSTITUTION_ID` = "+IDS);
						 if(i==1)
						 {
							 JOptionPane.showMessageDialog(null,
										"Address Edited",
				...
				catch(Exception ex)
				{
				 ...
				 else
				 {
					 ...
					 while(upResult.next())
					 {
						 i=du.ddlOperation("UPDATE `institution` SET `ADDRESS_ID` = '"+upResult.getString("ADDRESS_ID")+"' WHERE `institution`.`INSTITUTION_ID` = "+IDS);
					 }
					 if(i==1)
					 {
						 JOptionPane.showMessageDialog(null,
